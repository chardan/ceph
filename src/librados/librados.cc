...
extern "C" int rados_conf_get_keys(rados_t cluster, char *buf, const size_t len, size_t *req_sz_out)
{
 std::vector<std::string> keys;

 try
  {     
        if(nullptr == buf || nullptr == req_sz_out)
         return -EINVAL;
        ...
        librados::cutil::flatten_to_cstring(keys, len, buf);
