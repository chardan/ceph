...
cdef extern from "rados/librados.h" nogil:
    ...
    int rados_conf_get_keys(rados_t cluster, char *buf, const size_t len, size_t *req_sz_out)
    ...
    def conf_get_keys(self):
        ...
        try:
            while True:
                ...
                with nogil:
                    ret = rados_conf_get_keys(self.cluster, ret_buf, ret_buf_len, &req_sz_out)

                if (0 == ret): 
                    return cstring_to_list(ret_buf, ret)

                if (-errno.ERANGE == ret):
                    ret_buf = <char *>realloc_chk(ret_buf, req_sz_out)
                    continue

                if (-errno.EINVAL == ret):
                    raise make_ex(ret, "invalid parameter")

                raise make_ex(ret, "invalid result from rados_conf_get_keys()")
