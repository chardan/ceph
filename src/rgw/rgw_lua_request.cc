...
struct BucketTagsTable : public EmptyMetaTable {
  static int IndexClosure(lua_State* L) {
    ...
    try {
      ...
      if (const auto tag_it = find_string_map_entry(tag_map, key);
          tag_it != tag_map.end()) {
...
struct BucketMetaTable : public EmptyMetaTable {
  static int IndexClosure(lua_State* L) {
    ...
    if (rgw::sal::Bucket::empty(bucket)) {
      ...
      if (const auto it = find_string_map_entry(s->bucket_attrs, RGW_ATTR_TAGS);
          it != s->bucket_attrs.end()) {
      ...
      if (const auto u = std::get_if<rgw_user>(&owner); u) {
      ...
      const rgw_owner& owner = bucket->get_owner();
      if (const auto a = std::get_if<rgw_account_id>(&owner); a) {
        pushstring(L, *a);
      } else {
        lua_pushnil(L);
      }
